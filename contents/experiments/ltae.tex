...
As described in Section \ref{sec:tempCNNDataPreparation}, we divided the dataset into three subsets: training (60\%), validation (20\%), and test (20\%). 
Necessary precautions were taken to ensure that each set had a similar class distribution and to avoid spatial autocorrelation.

The model was implemented using the PyTorch framework. For the purpose of embedding the input images, the Pixel Set Encoder (PSE) was used, which has previously been shown to be a suitable technique for satellite image datasets \cite{garnot2020satellite}.

While the PSE method worked well for the original dataset, we developed a new encoder, called the Dense Encoder (DE), that better suited the needs of our dataset.
